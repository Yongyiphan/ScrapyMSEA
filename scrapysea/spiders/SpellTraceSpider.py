...
DLog = CL.Create_Log("ST_Spider", logTo="SpellTrace.log", propagate=False)
...
class ScrollTraceSpider(scrapy.Spider):
    ...
    def SpellTraceCost(self, response, Category):
        ...
        header = utils.RemoveDeli(header, {" cost": ""})
        ...
        for tr in stat_table.xpath(".//tr"):
            ...
            for n, td in enumerate(tds):  # Every Item
                ...
                try:
                    ...
                    DLog.info(
                        f"{Category}:{ItemGroup}({Success}) at {MinMaxL[n][0]}~{MinMaxL[n][-1]}"
                    )
                    ST_Bar.update(1)
                    yield scrolli.load_item()
                except IndexError:
                    DLog.error(f"Failed at {ItemGroup}({Success})")
                ...
